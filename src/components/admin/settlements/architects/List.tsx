...
export function ArchitectsList({
  architects,
  settlementId,
  getSettlement,
}: ArchitectsListProps) {
  ...
  async function getAvailableArchitects() {
    ...
    if (responseArchitects) {
      const filteredArchitects = responseArchitects.filter(
        (responseArchitect) =>
          !architects
  ...
  return (
    <div
      className={`grid transition-filter ${
        loading ? 'blur-sm pointer-events-none' : ''
      }`}
    >
      <div className='flex flex-col'>
        {architects &&
          sortAlphabetically(architects).map((architect: Architect) => (
      ...
      <div>
        <label htmlFor='settlementArchitectSelect' className='mb-1 block'>
          Architekt*in hinzufügen:
        </label>
