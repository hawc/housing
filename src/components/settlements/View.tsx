...
export function Settlement({ settlement }: SettlementProps) {
  ...
  return (
    <>
      {settlement.location &&
        settlement.location.lat > 0 &&
        settlement.location.lng > 0 && (
      ...
      <div className='relative'>
        ...
        <Box
          ghost
          className='py-0 md:p-0 mb-4 flex-row justify-between items-end'
        >
        ...
        <Container>
          {settlement.description && (
            <Container>
              <Box ghost className='mb-4'>
                <div
                  dangerouslySetInnerHTML={{ __html: settlement.description }}
                ></div>
              </Box>
            </Container>
          )}
          ...
          {settlement.architects.length > 0 && (
            <Container>
              <Box>
                <Headline className='inline-block' type='h2'>
                  {settlement.architects.length > 1
                    ? 'Architekt*innen'
                    : 'Architekt*in'}
                </Headline>
                <div
                  className={
                    settlement.architects.length > 3
                      ? 'md:columns-2'
                      : 'md:columns-1'
                  }
                >
                  {sortByRole(settlement.architects).map(
                    (architect: Architect) => (
                      <div key={architect.id}>
                        <LinkElement href={`/architekten/${architect.slug}`}>
                          {architect.name}
                        </LinkElement>
                        {architect.role ? (
                          <span className='font-thin tracking-wide'>{` (${architect.role})`}</span>
                        ) : (
                          ''
                        )}
                      </div>
                      ...
                      {(resource.license && resource.copyright && (
                        <>
                          <CopyrightIcon
                            ...
                            size={17}
                          />{' '}
                          {resource.copyright}, {resource.license}
                        </>
                      )) ||
