...
export async function generateMetadata(props): Promise<Metadata> {
  ...
  return {
    title: 'Ressourcen',
    alternates: searchParams.from ? { canonical: `${process.env.BASE_URL}/ressourcen` } : undefined
  };
}
...
async function getPlatforms() {
  ...
  return sortAlphabetically(platforms.filter(platform => !filteredResouces.includes(platform.name)));
}
        ...
        <Box ghost>
          ...
          {platforms.map(platform => (
            <Box key={platform.id}>
              ...
              <p  className="mt-2">
                <Link href={platform.url} arrow highlight>zu {platform.urlIdentifier}</Link>
              </p>
            </Box>
          ))}
      </section>
