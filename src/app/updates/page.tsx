          ...
          <div className='flex flex-col gap-6'>
            {sortedUpdates.map(([date, update]) => (
              <div key={date}>
                ...
                {update.map(update => (
                  <div key={update.slug} className='flex sm:flex-row flex-col sm:gap-2 leading-relaxed mt-4 sm:mt-0'>
                    <Link href={update.type === 'architect' ? `/architekten/${update.slug}` : `/siedlungen/${update.slug}`}>{update.name}</Link>
                    <span className="sr-only">,</span>
                    {'location' in update && <><span className='font-thin tracking-wide'>{update.location?.city}</span><span className="sr-only">, </span></>}
                    <span className='text-gray-500 font-thin tracking-wide'>({update.changeType === 'updated' ? 'zuletzt aktualisiert' : 'neu hinzugefügt'})</span>
                  </div>
                ))}
              </div>
            ))}
